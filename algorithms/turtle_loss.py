...
from .modules.meta_loss import MetaLossNetwork

class TurtleLoss(Algorithm):
    ...
    def __init__(self, input_type, history, beta, grad_clip, layers, act, 
        ...
        **kwargs : dict
        ...
        # Create loss network and parameters
        self.loss_network = MetaLossNetwork(self.baselearner.eval_classes*2, hidden_dim=[40,40])
        self.loss_params = [p.clone().detach().to(self.dev) for p in self.loss_network.parameters()]
        for p in self.loss_params:
            p.requires_grad = True
        self.class_map = {x:x for x in range(self.baselearner.eval_classes)}
        ...
        self.meta_params += self.loss_params
    ...
    def _deploy(self, train_x, train_y, test_x, test_y):
        ...
        for step in range(self.T):
            ...
            X, grads = get_init_info(self.baselearner, train_x, train_y, weights=fast_weights,
                                 ...
                                 input_code=self.input_code, grad_clip=self.grad_clip, meta_loss=True, 
                                 class_map=self.class_map, loss_net=self.loss_network, loss_params=self.loss_params)
    ...
    def evaluate(self, train_x, train_y, test_x, test_y, **kwargs):
        ...
        if self.operator == min:
            ...
            return accuracy(preds, test_y), None
